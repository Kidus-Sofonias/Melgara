...
function useOreGeometry(seed, detail, baseColor, veinColor, patina) {
  return useMemo(() => {
    ...
    const cDark = new THREE.Color("#0a0908");
    const v = new THREE.Color();

    for (let i = 0; i < pos.count; i++) {
      base.fromBufferAttribute(pos, i);
      const len = base.length();
      const nx = base.x / len, ny = base.y / len, nz = base.z / len;
      ...
      const squash = 0.92 + 0.08 * Math.abs(ny);
      const rr = len * (1 + d * 0.4) * squash;
      pos.setXYZ(i, nx * rr, ny * rr * 0.96, nz * rr);
      const vein = THREE.MathUtils.smoothstep(0.42, 0.92,
        Math.abs(veinNoise(nx * 2.8 + 40, ny * 2.8, nz * 2.8)));
      v.copy(cBase).lerp(cVein, vein * 0.85);
      if (cPatina) {
        const pat = THREE.MathUtils.smoothstep(0.52, 0.95,
          Math.abs(veinNoise(nx * 1.4 + 17, ny * 1.4, nz * 1.4)));
        v.lerp(cPatina, pat * 0.5);
      }
      const lift = THREE.MathUtils.smoothstep(-0.35, 0.6, d);
      v.lerp(cDark, (1 - lift) * 0.28);
      colors[i * 3] = v.r; colors[i * 3 + 1] = v.g; colors[i * 3 + 2] = v.b;
    }
  ...
  return useMemo(() => {
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        const i = (y * size + x) * 4;
        let n = noise(x / 26, y / 26, 0) * 0.6
              + noise(x / 11 + 7, y / 11, 0) * 0.3
              + noise(x / 4 + 3, y / 4, 0) * 0.1;
        const val = Math.min(255, Math.max(0, Math.round((n * 0.5 + 0.5) * 255)));
...
/* ================================================================
   ...
   ================================================================ */
function ProceduralOre({ color, veinColor, patina, roughness, metalness, seed, detail, lite, photo }) {
  ...
  useEffect(() => {
    ...
    loader.load(photo, (tex) => {
      ...
      tex.anisotropy = 4;
      setPhotoTex(tex);
    }, undefined, () => { if (!cancelled) setPhotoFailed(true); });
    return () => { cancelled = true; setPhotoTex((t) => { if (t) t.dispose(); return null; }); };
  }, [photo, lite, fast]);
  ...
  useEffect(() => () => { geometry.dispose(); if (bumpTex) bumpTex.dispose(); }, [geometry, bumpTex]);

  return (
    <mesh key={triplanar ? "photo" : "proc"} geometry={geometry} castShadow receiveShadow>
      <meshPhysicalMaterial
        vertexColors color="#ffffff"
        roughness={roughness} metalness={metalness}
        envMapIntensity={1.7}
        clearcoat={metalness > 0.7 ? 0.45 : 0.12} clearcoatRoughness={0.3}
        bumpMap={bumpTex} bumpScale={0.012} roughnessMap={bumpTex}
...
/* ================================================================
   ...
   ================================================================ */
function Ore({
  color, veinColor = "#f0a35c", patina = null,
  roughness, metalness, seed, intensity = 1,
  idleSpeed = 0.12, detail = 48, lite = false,
  photo = null, modelPath = null,
}) {
  ...
  useFrame((state, delta) => {
    ...
    if (!reduced) {
      const scrollP = typeof window !== "undefined"
        ? window.scrollY / Math.max(document.documentElement.scrollHeight - window.innerHeight, 1) : 0;
      const targetY = scrollP * Math.PI * 2 + state.clock.elapsedTime * idleSpeed * intensity;
      g.rotation.y += (targetY - g.rotation.y) * (1 - Math.pow(0.001, delta));
    }
    const baseY = aspect < 0.75 ? -0.55 : 0;
    g.position.y = baseY + (reduced ? 0 : Math.sin(state.clock.elapsedTime * 0.6 + seed) * 0.12);
  });

  const proceduralFallback = (
    <ProceduralOre
      color={color} veinColor={veinColor} patina={patina}
      roughness={roughness} metalness={metalness}
      seed={seed} detail={detail} lite={lite} photo={photo}
    />
  );
...
function Dust({ count = 90 }) {
  ...
  useFrame((state) => { if (ref.current) ref.current.rotation.y = state.clock.elapsedTime * 0.012; });
  return (
    <points ref={ref} geometry={geometry}>
      <pointsMaterial size={0.035} color="#e8944f" transparent opacity={0.55} sizeAttenuation depthWrite={false} />
    </points>
  );
}
...
function Plinth() {
  return (
    <mesh position={[0, -2.1, 0]} receiveShadow>
      ...
      <meshStandardMaterial color="#1a1612" roughness={0.5} metalness={0.45} />
    </mesh>
  );
}
...
function CameraRig({ enabled }) {
  ...
  useFrame((state, delta) => {
    ...
    camera.position.z += ((aspect < 0.75 ? 7.6 : aspect < 1.15 ? 6.3 : 5.2) - camera.position.z) * k;
...
function StaticFallback({ color }) {
  return (
    <div className="sk-ore-loading" aria-hidden="true" style={{
      width: "100%", height: "100%",
      background: `radial-gradient(circle at 50% 40%, ${color}55, #151210 72%)`,
      borderRadius: "inherit",
      position: "relative",
      overflow: "hidden",
    }}>
      <div className="sk-ore-shimmer" style={{
        position: "absolute",
        inset: 0,
...
export default function OreCanvas3D({
  color = "#c8793e", veinColor = "#f0a35c", patina = null,
  roughness = 0.35, metalness = 0.85, seed = 7,
  intensity = 1, detail = 48, dpr = 2, lite = false,
  photo = null, modelPath = null, className = "", style,
}) {
  return (
    <OreErrorBoundary fallback={<StaticFallback color={color} />}>
      ...
      >
        <Suspense fallback={null}>
          ...
          <directionalLight position={[4, 6, 4]} intensity={lite ? 2.2 : 2.0} color="#ffd9b0" />
          <directionalLight position={[-5, -2, -4]} intensity={0.7} color="#8a6bd1" />
          <pointLight position={[0, -3, 2]} intensity={0.9} color="#c8793e" />
          <Ore
            color={color} veinColor={veinColor} patina={patina}
            roughness={roughness} metalness={metalness} seed={seed}
            intensity={intensity} detail={detail} lite={lite}
            photo={photo} modelPath={modelPath}
          />
          <Plinth />
          <Dust count={lite ? 30 : detail >= 48 ? 90 : 45} />
          <ContactShadows position={[0, -1.96, 0]} opacity={lite ? 0.5 : 0.7} scale={6} blur={2.6} far={3} color="#000000" />
          {!lite && <CameraRig enabled />}
          {!lite && (
            <Environment resolution={256} frames={1}>
              <Lightformer intensity={5} position={[0, 4, 6]} scale={[8, 3, 1]} color="#ffd9b0" />
              <Lightformer intensity={2.5} position={[-6, 1, 2]} rotation-y={Math.PI / 2} scale={[5, 2, 1]} color="#9a86d6" />
              <Lightformer intensity={3.5} position={[6, -1, 1]} rotation-y={-Math.PI / 2} scale={[4, 2, 1]} color="#e8944f" />
              <Lightformer intensity={2} position={[0, -5, 0]} rotation-x={Math.PI / 2} scale={[10, 10, 1]} color="#3a2a1c" />
            </Environment>
          )}
        </Suspense>
      </Canvas>
